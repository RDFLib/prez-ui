...
function configByType(type: string) {
    ...
    switch (type) {
        case qname("dcat:Catalog"):
            ...
            childrenConfig.value.childrenTitle = "Resources";
            break;
        ...
        case qname("dcat:Dataset"):
            ...
            childrenConfig.value.showButton = true;
            childrenConfig.value.buttonTitle = "Members";
            childrenConfig.value.buttonLink = "/collections";
            break;
        case qname("geo:FeatureCollection"):
            ...
            childrenConfig.value.showButton = true;
            childrenConfig.value.buttonTitle = "Members";
            childrenConfig.value.buttonLink = "/items";
            break;
        ...
        case qname("skos:Collection"):
            childrenConfig.value.showChildren = true;
            childrenConfig.value.childrenTitle = "Concepts";
            break;
...
function getBreadcrumbs(): Breadcrumb[] {
    // if /object, then use basic home/object/<object>
    // else, use type/path to determine breadcrumbs
        ...
        pathSegments.forEach((pathSegment, index) => {
            ...
            switch (pathSegment) {
                ...
                case "collections":
                    ...
                    break;
                case "items":
                    crumbs.push({ name: "Features", url: "/s/items" });
                    break;
                case "vocab":
...
function getChildren() {
    if (item.value.type === qname("skos:ConceptScheme")) {
        ...
        // check against list of child predicates, set which one matches
        DEFAULT_CHILDREN_PREDICATES.forEach(predicate => {
            const objects = store.value.getObjects(namedNode(item.value.iri), namedNode(predicate), null);
            if (objects.length > 0) {
                childrenPredicate.value = predicate;
                hiddenPredicates.value.push(predicate);
            }
...
onBeforeMount(() => {
    ...
    if (route.path.startsWith("/c/")) {
        ...
        // get type from regex match on path
...
onMounted(() => {
    ...
    doRequest(`${apiBaseUrl}${route.fullPath}`, () => {
        ...
        // check if using other profile, then redirect to API
        if (route.query && route.query._profile) {
            if (route.query._mediatype || ![defaultProfile.value.token, ALT_PROFILES_TOKEN].includes(route.query._profile as string)) {
...
<template>
    ...
    <template v-else>
        <PropTable v-if="properties.length > 0" :item="item" :properties="properties" :blankNodes="blankNodes" :prefixes="prefixes" :hiddenPreds="hiddenPredicates">
            ...
            <template v-else-if="childrenConfig.showButton" #bottom>
                <tr>
                    <th>{{ childrenConfig.childrenTitle }}</th>
                    <td>
