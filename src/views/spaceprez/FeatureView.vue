...
//     a prof:Profile ;
//     dcterms:description "feature feature Vocabulary (DCAT) is a W3C-authored RDF vocabulary designed to facilitate interoperability between data features" ;
...
onMounted(() => {
    doRequest(`${apiBaseUrl}/s/datasets/${route.params.datasetId}/collections/${route.params.featureCollectionId}/items/${route.params.featureId}`, () => {
        ...
        const subject = store.value.getSubjects(namedNode(qname("a")), namedNode("http://www.opengis.net/ont/geosparql#Feature"))[0];
