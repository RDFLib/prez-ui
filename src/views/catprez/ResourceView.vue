...
onMounted(() => {
    doRequest(`${apiBaseUrl}/c/catalogs/${route.params.catalogId}/${route.params.resourceId}`, () => {
        ...
        const subject = store.value.getSubjects(namedNode(qname("a")), namedNode("http://www.w3.org/ns/dcat#Resource"))[0];
    ...
    ui.breadcrumbs = [{ name: "CatPrez", url: "/c" }, { name: "Catalogs", url: "/c/catalogs" }, { name: "Catalog", url: `/c/catalogs/${route.params.catalogId}` }, { name: resource.value.title, url: route.path }];
});
</script>
